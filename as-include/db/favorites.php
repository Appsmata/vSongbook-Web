...
/*
	...
	example, it could include the activity on a particular song, or the activity by a particular user.
	...
	/as-include/app/options.php) since it's no big deal to write 10 rows to a table. Recall that whenever an event stream
...
function as_db_favorite_create($userid, $entitytype, $entityid)
{
	...
	if ($useshared) {
		...
		as_db_query_sub(
			...
			'SELECT #, entitytype, entityid, songid, lastpostid, updatetype, lastuserid, updated FROM ' .
