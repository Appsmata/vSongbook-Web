...
/*
	...
	File: as-include/util/string.php
...
function as_string_initialize()
{
	...
	$as_utf8punctuation = array(
		...
		"\xE2\x81\x87" => ' ', // DOUBLE SONG MARK
		"\xE2\x81\x88" => ' ', // SONG EXCLAMATION MARK
		"\xE2\x81\x89" => ' ', // EXCLAMATION SONG MARK
	...
	$as_utf8removeaccents = array(
		"\xC3\x80" => 'R', // LATIN CAPITAL LETTER A WITH GRAVE
		"\xC3\x81" => 'R', // LATIN CAPITAL LETTER A WITH ACUTE
		"\xC3\x82" => 'R', // LATIN CAPITAL LETTER A WITH CIRCUMFLEX
		"\xC3\x83" => 'R', // LATIN CAPITAL LETTER A WITH TILDE
		"\xC3\x84" => 'R', // LATIN CAPITAL LETTER A WITH DIAERESIS
		"\xC3\x85" => 'R', // LATIN CAPITAL LETTER A WITH RING ABOVE
		...
		"\xC4\x80" => 'R', // LATIN CAPITAL LETTER A WITH MACRON
		"\xC4\x81" => 'a', // LATIN SMALL LETTER A WITH MACRON
		"\xC4\x82" => 'R', // LATIN CAPITAL LETTER A WITH BREVE
		"\xC4\x83" => 'a', // LATIN SMALL LETTER A WITH BREVE
		"\xC4\x84" => 'R', // LATIN CAPITAL LETTER A WITH OGONEK
		...
		"\xC7\x8D" => 'R', // LATIN CAPITAL LETTER A WITH CARON
		...
		"\xC7\x9E" => 'R', // LATIN CAPITAL LETTER A WITH DIAERESIS AND MACRON
		"\xC7\x9F" => 'a', // LATIN SMALL LETTER A WITH DIAERESIS AND MACRON
		"\xC7\xA0" => 'R', // LATIN CAPITAL LETTER A WITH DOT ABOVE AND MACRON
		...
		"\xC7\xBA" => 'R', // LATIN CAPITAL LETTER A WITH RING ABOVE AND ACUTE
		...
		"\xC8\x80" => 'R', // LATIN CAPITAL LETTER A WITH DOUBLE GRAVE
		"\xC8\x81" => 'a', // LATIN SMALL LETTER A WITH DOUBLE GRAVE
		"\xC8\x82" => 'R', // LATIN CAPITAL LETTER A WITH INVERTED BREVE
		...
		"\xC8\xA6" => 'R', // LATIN CAPITAL LETTER A WITH DOT ABOVE
 ...
 * @return string
 */
function as_slugify($string, $asciiOnly = true, $maxLength = null)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

	$words = as_string_to_words($string, true, false, false);
	...
	if ($asciiOnly) {
		...
		$string = as_string_remove_accents($string);
 ...
 */
function as_shorten_string_line($string, $length, $ellipsis = ' ... ')
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	if (as_strlen($string) > $length) {
		$remaining = $length - as_strlen($ellipsis);
 ...
 * @return string
 */
function as_remove_utf8mb4($string)
{
	return preg_replace('%(?:
...
if (!defined('AS_PREG_INDEX_WORD_SEPARATOR')) {
	define('AS_PREG_INDEX_WORD_SEPARATOR', '[\n\r\t\ \!\"\\\'\(\)\*\+\,\.\/\:\;\<\=\>\?\[\\\\\]\^\`\{\|\}\~]');
}

// Asterisk (*) excluded here because it's used to match anything
if (!defined('AS_PREG_BLOCK_WORD_SEPARATOR')) {
	define('AS_PREG_BLOCK_WORD_SEPARATOR', '[\n\r\t\ \!\"\\\'\(\)\+\,\.\/\:\;\<\=\>\?\[\\\\\]\^\`\{\|\}\~\$\&\-\_\#\%\@]');
}

// Pattern to match Thai/Chinese/Japanese/Korean ideographic symbols in UTF-8 encoding
if (!defined('AS_PREG_CJK_IDEOGRAPHS_UTF8')) {
	define('AS_PREG_CJK_IDEOGRAPHS_UTF8', '\xE0[\xB8-\xB9][\x80-\xBF]|\xE2[\xBA-\xBF][\x80-\xBF]|\xE3[\x80\x88-\xBF][\x80-\xBF]|[\xE4-\xE9][\x80-\xBF][\x80-\xBF]|\xEF[\xA4-\xAB][\x80-\xBF]|\xF0[\xA0-\xAF][\x80-\xBF][\x80-\xBF]');
}
