...
/*
	...
	Description:  Database functions for changing a song, review or comment
...
/**
 * Update the selected review in the database for $songid to $selchildid, and optionally record that $lastuserid did it from $lastip
 * @param $songid
 * @param $selchildid
 * @param $lastuserid
 * @param $lastip
 */
function as_db_post_set_selchildid($songid, $selchildid, $lastuserid = null, $lastip = null)
{
	as_db_query_sub(
...
/**
 * Set $songid to be closed by post $closedbyid (null if not closed) in the database, and optionally record that
 ...
 * @param $lastip
 */
function as_db_post_set_closed($songid, $closedbyid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		as_db_query_sub(
			"UPDATE ^posts SET closedbyid=#, updated=NOW(), updatetype=$, lastuserid=$, lastip=UNHEX($) WHERE postid=#",
			$closedbyid, AS_UPDATE_CLOSED, $lastuserid, bin2hex(@inet_pton($lastip)), $songid
		);
	} else {
		as_db_query_sub(
 ...
 * @param $name
 */
function as_db_post_set_content($postid, $number, $songkey, $title, $content, $format, $tagstring, $notify, $lastuserid = null, $lastip = null, $updatetype = AS_UPDATE_CONTENT, $name = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		// use COALESCE() for name since $name=null means it should not be modified (for backwards compatibility)
		as_db_query_sub(
			'UPDATE ^posts SET number=#, songkey=$, title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$, updated=NOW(), updatetype=$, lastuserid=$, lastip=UNHEX($) WHERE postid=#',
			$number, $songkey, $title, $content, $format, $tagstring, $name, $notify, $updatetype, $lastuserid, bin2hex(@inet_pton($lastip)), $postid
		);
	} else {
		as_db_query_sub(
			'UPDATE ^posts SET number=#, songkey=$, title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$ WHERE postid=#',
			$number, $songkey, $title, $content, $format, $tagstring, $name, $notify, $postid
		);
	}
}
...
/**
 * Deletes post $postid from the database (will also delete any thumbs on the post due to foreign key cascading)
 * @param $postid
 */
...
/**
 * Return the array $postids containing only those elements which are the postid of a song in the database
 * @param $postids
 * @return array
 */
function as_db_posts_filter_q_postids($postids)
{
	if (count($postids)) {
		return as_db_read_all_values(as_db_query_sub(
			"SELECT postid FROM ^posts WHERE type='S' AND postid IN (#)",
			$postids
		));
	}
...
function as_db_flaggedcount_update()
{
	if (as_should_update_counts()) {
		as_db_query_sub(
			...
			"WHERE flagcount > 0 AND type IN ('S', 'R', 'C') " .
