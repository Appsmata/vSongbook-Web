...
/**
 * Return number of songs in the database in $categoryid exactly, and not one of its subcategories
 * @param $categoryid
 * @return mixed|null
 */
...
{
	return as_db_read_one_value(as_db_query_sub(
		"SELECT COUNT(*) FROM ^posts WHERE categoryid<=># AND type='S'",
		$categoryid
	));
}
...
function as_db_get_user_visible_postids($userid)
{
	return as_db_read_all_values(as_db_query_sub(
		"SELECT postid FROM ^posts WHERE userid=# AND type IN ('S', 'R', 'C', 'S_QUEUED', 'R_QUEUED', 'C_QUEUED')",
		$userid
	));
}
...
{
	return as_db_read_all_values(as_db_query_sub(
		"SELECT postid FROM ^posts WHERE createip=UNHEX($) AND type IN ('S', 'R', 'C', 'S_QUEUED', 'R_QUEUED', 'C_QUEUED')",
