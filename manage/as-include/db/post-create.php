 ...
 * @return mixed
 */
function as_db_post_create($type, $parentid, $userid, $cookieid, $ip, $number, $songkey, $title, $content, $format, $tagstring, $notify, $categoryid = null, $name = null)
{
	as_db_query_sub(
		'INSERT INTO ^posts (categoryid, type, parentid, userid, cookieid, createip, number, songkey, title, content, format, tags, notify, name, created) ' .
		'VALUES (#, $, #, $, #, UNHEX($), #, $, $, $, $, $, $, $, NOW())',
		$categoryid, $type, $parentid, $userid, $cookieid, bin2hex(@inet_pton($ip)), $number, $songkey, $title, $content, $format, $tagstring, $notify, $name
	);
