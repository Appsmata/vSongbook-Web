...
define('AS_USER_FLAGS_MUST_APPROVE', 512); // @deprecated
...
if (AS_FINAL_EXTERNAL_USERS) {
	...
	if (defined('AS_FINAL_WORDPRESS_INTEGRATE_PATH')) {
		require_once AS_INCLUDE_DIR . 'util/external-users-wp.php';
	} elseif (defined('AS_FINAL_JOOMLA_INTEGRATE_PATH')) {
		require_once AS_INCLUDE_DIR . 'util/external-users-joomla.php';
	} else {
	...
	function as_get_logged_in_user_cache()
	{
		...
		if (!isset($as_cached_logged_in_user)) {
			...
			if (isset($user)) {
				$user['flags'] = isset($user['blocked']) ? AS_USER_FLAGS_USER_BLOCKED : 0;
				$as_cached_logged_in_user = $user;
			} else
				$as_cached_logged_in_user = false;
		}
	...
	function as_start_session()
	{
		...
		@ini_set('session.use_trans_sid', false); // sessions need cookies to work, since we redirect after signin
	...
	function as_session_var_suffix()
	{
		...
		if (!$as_session_suffix) {
			...
			$as_session_suffix = md5(AS_FINAL_MYSQL_HOSTNAME . '/' . AS_FINAL_MYSQL_USERNAME . '/' . AS_FINAL_MYSQL_PASSWORD . '/' . AS_FINAL_MYSQL_DATABASE . '/' . $prefix);
		}
	...
	function as_set_session_user($userid, $source)
	{
		...
		// prevents one account on a shared server being able to create a log in a user to APS on another account on same server
	 ...
	 * $remember states if 'Remember me' was checked in the signin form.
	 * @param $userid
	 * @param string $handle
	 * @param bool $remember
	 * @param $source
	...
	function as_set_logged_in_user($userid, $handle = '', $remember = false, $source = null)
	{
		...
		if (isset($userid)) {
			...
			as_report_event('u_signin', $userid, $userinfo['handle'], as_cookie_get());
			...
			as_report_event('u_signout', $olduserid, $oldhandle, as_cookie_get());
		}
	}
	...
	function as_log_in_external_user($source, $identifier, $fields)
	{
		...
		$users = as_db_user_signin_find($source, $identifier);
		$countusers = count($users);

		if ($countusers > 1)
			as_fatal_error('External signin mapped to more than one user'); // should never happen
		...
		if ($countusers) // user exists so log them in
			...
			as_db_user_signin_sync(true);

			$users = as_db_user_signin_find($source, $identifier); // check again after table is locked

			if (count($users) == 1) {
				as_db_user_signin_sync(false);
				as_set_logged_in_user($users[0]['userid'], $users[0]['handle'], false, $source);
				...
				if (strlen(@$fields['email'])) { // remove email address if it will cause a duplicate
					...
					if (count($emailusers)) {
						as_redirect('signin', array('e' => $fields['email'], 'ee' => '1'));
						unset($fields['email']);
	...
	function as_get_logged_in_userid()
	{
		...
		if (!$as_logged_in_userid_checked) { // only check once
			...
			if (isset($sessionuserid)) // check verify code matches
				if (!hash_equals(as_session_verify_code($sessionuserid), @$_SESSION['as_session_verify_' . $suffix]))
					as_clear_session_user();
	...
	function as_get_logged_in_user_cache()
	{
		...
		if (!isset($as_cached_logged_in_user)) {
			...
			if (isset($userid)) {
				require_once AS_INCLUDE_DIR . 'db/selects.php';
				$as_cached_logged_in_user = as_db_get_pending_result('signedinuser', as_db_user_account_selectspec($userid, true));
				...
				if ($as_cached_logged_in_user['points'] === null) {
					require_once AS_INCLUDE_DIR . 'db/points.php';
					require_once AS_INCLUDE_DIR . 'db/users.php';

					as_db_points_update_ifuser($userid, null);
					as_db_uapprovecount_update();
					$as_cached_logged_in_user = as_db_single_select(as_db_user_account_selectspec($userid, true));
				}
	...
	function as_get_logged_in_user_field($field)
	{
		...
		$usercache = as_get_logged_in_user_cache();
	 ...
	 */
	function as_get_one_user_html($handle, $microdata = false, $favorited = false)
	{
		if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	 ...
	 */
	function as_get_user_avatar_source($flags, $email, $blobId)
	{
		if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

		if (as_opt('avatar_allow_gravatar') && (($flags & AS_USER_FLAGS_SHOW_GRAVATAR) > 0) && isset($email)) {
			return 'gravatar';
		} elseif (as_opt('avatar_allow_upload') && (($flags & AS_USER_FLAGS_SHOW_AVATAR) > 0) && isset($blobId)) {
			return 'local-user';
	 ...
	 */
	function as_get_user_avatar_url($flags, $email, $blobId, $size = null, $absolute = false)
	{
		if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

		$avatarSource = as_get_user_avatar_source($flags, $email, $blobId);
	...
	function as_get_user_avatar_html($flags, $email, $handle, $blobId, $width, $height, $size, $padding = false)
	{
		...
		return sprintf('<a href="%s" class="as-avatar-link">%s</a>', as_path_html('user/' . $handle), $html);
	}
	...
	/**
	 * Return an array of links to signin, signup, email confirm and signout pages (if not using single sign-on integration)
	 * @param $rooturl
	 * @param $tourl
	 * @return array
	 */
	function as_get_signin_links($rooturl, $tourl)
	{
		return array(
			'signin' => as_path('signin', isset($tourl) ? array('to' => $tourl) : null, $rooturl),
			'signup' => as_path('signup', isset($tourl) ? array('to' => $tourl) : null, $rooturl),
			'confirm' => as_path('confirm', null, $rooturl),
			'signout' => as_path('signout', null, $rooturl),
		);
	}
 ...
 * @return mixed|null
 */
function as_userid_to_handle($userid)
{
	$handles = as_userids_to_handles(array($userid));
 ...
 * @param string $limitaction Constant from /as-include/app/limits.php to check against user or IP rate limits.
 * @param int $userlevel A AS_USER_LEVEL_* constant to consider the user at a different level to usual (e.g. if
 ...
 *   'signin' => the user should signin or signup
 ...
 */
function as_user_permit_error($permitoption = null, $limitaction = null, $userlevel = null, $checkblocks = true, $userfields = null)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
...
/**
 * Check whether user can perform $permitoption. Result as for as_user_permit_error(...).
 *
...
/**
 * Check whether user can reach the permission level. Result as for as_user_permit_error(...).
 *
...
function as_permit_value_error($permit, $userid, $userlevel, $userflags)
{
	...
	if (!isset($userid) && $permit < AS_PERMIT_ALL)
		return 'signin';

	$levelError =
		($permit <= AS_PERMIT_SUPERS && $userlevel < AS_USER_LEVEL_SUPER) ||
		($permit <= AS_PERMIT_ADMINS && $userlevel < AS_USER_LEVEL_ADMIN) ||
		($permit <= AS_PERMIT_MODERATORS && $userlevel < AS_USER_LEVEL_MODERATOR) ||
		($permit <= AS_PERMIT_EDITORS && $userlevel < AS_USER_LEVEL_EDITOR) ||
		($permit <= AS_PERMIT_EXPERTS && $userlevel < AS_USER_LEVEL_EXPERT);

	if ($levelError)
		return 'level';

	if (isset($userid) && ($userflags & AS_USER_FLAGS_USER_BLOCKED))
		return 'userblock';

	if ($permit >= AS_PERMIT_USERS)
		return false;

	if ($permit >= AS_PERMIT_CONFIRMED) {
		$confirmed = ($userflags & AS_USER_FLAGS_EMAIL_CONFIRMED);
		// not currently supported by single sign-on integration; approved users and above don't need confirmation
		if (!AS_FINAL_EXTERNAL_USERS && as_opt('confirm_user_emails') && $userlevel < AS_USER_LEVEL_APPROVED && !$confirmed) {
			return 'confirm';
		}
	} elseif ($permit >= AS_PERMIT_APPROVED) {
		// check user is approved, only if we require it
		if (as_opt('moderate_users') && $userlevel < AS_USER_LEVEL_APPROVED) {
			return 'approve';
		}
	}
 ...
 *
 * Possible results:
 * 'signin' => captcha required because the user is not logged in
 ...
 *
 * Possible results:
 * 'signin' => moderation required because the user is not logged in
...
function as_user_moderation_reason($userlevel = null)
{
	...
	if ($userlevel < AS_USER_LEVEL_EXPERT && as_user_permit_error('permit_moderate')) {
...
function as_check_form_security_code($action, $value)
{
	...
	if (!empty($reportproblems) && AS_DEBUG_PERFORMANCE) {
		@error_log(
			'PHP vSongBook form security violation for ' . $action .
